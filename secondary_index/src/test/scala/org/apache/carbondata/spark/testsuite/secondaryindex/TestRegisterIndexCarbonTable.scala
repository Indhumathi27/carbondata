...
class TestRegisterIndexCarbonTable extends QueryTest with BeforeAndAfterAll {
  ...
  test("register tables test") {
    sql("drop database if exists carbon cascade")
    sql(s"create database carbon location '${TestQueryExecutor.location}'")
    sql("use carbon")
    ...
    backUpData(TestQueryExecutor.location, "carbontable")
    backUpData(TestQueryExecutor.location, "index_on_c3")
    sql("drop table carbontable")
    restoreData(TestQueryExecutor.location, "carbontable")
    restoreData(TestQueryExecutor.location, "index_on_c3")

...
object DataMapUtil {
  ...
  def getPartitionInfo(usePartitioning: Boolean,
      parentTable: CarbonTable): Seq[String] = {
    ...
    val parentPartitionColumns = if (!usePartitioning) {
      ...
      parentTable.getPartitionInfo.getColumnSchemaList.asScala.map(_.getColumnName)
    } else {
      Seq()
    }

...
class TestNonTransactionalCarbonTablewithAvroDataType extends QueryTest with BeforeAndAfterAll {
  ...
  test("test Union with struct of array") {
    ...
    sql(
      s"""CREATE EXTERNAL TABLE sdkOutputTable(id decimal(4,3)) STORED BY
         |'carbondata' LOCATION
         |'$writerPath' """.stripMargin)
    checkAnswer(sql("select * from sdkOutputTable"),Seq(Row(Row(Row(null,mutable.WrappedArray.make(Array(Row(Row("abc")))))))))
  }
  ...
  test("test union type with Array and Struct") {
    ...
    checkAnswer(sql("select * from sdkOutputTable"),Seq(Row(Row(Row(null, mutable.WrappedArray.make(Array(Row(Row("abc"),Row(null, null,"bcd"),Row("cde")))))))))
  }

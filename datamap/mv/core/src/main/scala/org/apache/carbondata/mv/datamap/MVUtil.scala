...
object MVUtil {
  ...
  private def getCarbonTable(logicalRelation: Seq[LogicalRelation],
    ...
    val relations = logicalRelation
      .filter(lr => lr.output
        .exists(attrRef => attrRef.name.equals(attr.name) && attrRef.exprId.equals(attr.exprId)))
  ...
  def getAllParentTables(sparkSession: SparkSession,
      selectTables: Seq[CatalogTable],
      parentTables: java.util.List[CarbonTable]): java.util.List[CarbonTable] = {

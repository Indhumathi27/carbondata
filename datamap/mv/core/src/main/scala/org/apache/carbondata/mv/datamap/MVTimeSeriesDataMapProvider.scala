...
class MVTimeSeriesDataMapProvider(mainTable: CarbonTable,
  ...
  override def initMeta(ctasSqlStatement: String): Unit = {
    ...
    val dmProviderName: String = dataMapSchema.getProviderName
    TimeSeriesUtil.validateTimeSeriesGranularity(dmProperties, dmProviderName)
    val details: Tuple2[String, String] = TimeSeriesUtil.getTimeSeriesGranularityDetails(
      dmProperties,
      dmProviderName)
    dmProperties.remove(details._1)
    super.setTimeSeriesFunction(Some(details._1))

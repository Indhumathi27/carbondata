...
class MVIncrementalLoadingTestcase extends QueryTest with BeforeAndAfterAll {
  ...
  test("test MV incremental loading with rename table on main table") {
    createTableFactTable("test_table")
    sql("drop datamap if exists datamap1")
    sql(
      "create datamap datamap1 using 'mv' as select empname, designation " +
    ...
    intercept[MalformedCarbonCommandException] {
      sql("alter table test_table rename to test_table3")
    }
    intercept[MalformedCarbonCommandException] {
      sql("alter table datamap1_table rename to datamap_table3")
    }
  }
  ...
  test("test MV incremental loading with update operation on main table") {
    ...
    sql("create table dimensiontable(b string, d string) stored by 'carbondata'")
    sql("insert into dimensiontable values('abc','ab')")
    sql("insert into dimensiontable values('bdr','bd')")
    ...
    sql("drop datamap if exists datamap1")
    sql(
      "create datamap datamap1 using 'mv' as select a, d, sum(c) " +
      "from main_table inner join dimensiontable on main_table.b=dimensiontable.b group by a,d")
    sql(s"rebuild datamap datamap1")
    var df = sql(
      s"""select a, d, sum(c) from main_table inner join dimensiontable on
         |main_table.b=dimensiontable.b group by a,d""".stripMargin)
    var analyzed = df.queryExecution.analyzed
    assert(verifyMVDataMap(analyzed, "datamap1"))
    checkAnswer(sql(
      " select a, d, sum(c) from main_table inner join dimensiontable on " +
      "main_table.b=dimensiontable.b group by a,d"),
      sql(
        " select a, d, sum(c) from testtable inner join dimensiontable on testtable" +
        ".b=dimensiontable.b group by a,d"))
    ...
    details.foreach(detail => if(detail.getDataMapName.equalsIgnoreCase("datamap1")) {
      ...
      assert(detail.getSegmentInfo.get("dimensiontable").isEmpty)
    })
    checkAnswer(sql("select * from main_table"), sql("select * from testtable"))
    sql(s"rebuild datamap datamap1")
    df = sql(
      s"""select a, d, sum(c) from main_table inner join dimensiontable on main_table
         |.b=dimensiontable.b group by a,d"""
        .stripMargin)
    analyzed = df.queryExecution.analyzed
    assert(verifyMVDataMap(analyzed, "datamap1"))
    checkAnswer(
      sql(" select a, d, sum(c) from main_table inner join dimensiontable on " +
          "main_table.b=dimensiontable.b group by a,d"),
      sql(" select a, d, sum(c) from testtable inner join dimensiontable on " +
          "testtable.b=dimensiontable.b group by a,d"))
  }
  ...
  test("test insert overwrite") {
    ...
    sql("create table dt(b string, d string) stored by 'carbondata'")
    sql("insert into dt values('abc','ab')")
    sql("insert into dt values('bdr','bd')")
    sql("drop datamap if exists datamap1")
    sql(
      "create datamap datamap1 using 'mv' as select a, d, sum(c) " +
      "from mt inner join dt on mt.b=dt.b group by a,d")
    sql(s"rebuild datamap datamap1")
    sql(" select a, d, sum(c) from mt inner join dt on mt.b=dt.b group by a,d").show(false)
    sql("insert overwrite table mt select 'd','abc',3")
    var details = DataMapStatusManager.readDataMapStatusDetails()
    details.foreach(detail => if(detail.getDataMapName.equalsIgnoreCase("datamap1")) {
      assert(detail.getSegmentInfo.get("mt").isEmpty)
      assert(detail.getSegmentInfo.get("dt").isEmpty)
    })
    sql(" select a, d, sum(c) from mt inner join dt on mt.b=dt.b group by a,d").show(false)
    sql(s"rebuild datamap datamap1")
    ...
    details.foreach(detail => if(detail.getDataMapName.equalsIgnoreCase("datamap1")) {
      ...
      assert(detail.getSegmentInfo.get("dt").contains("0"))
      assert(detail.getSegmentInfo.get("dt").contains("0"))
    })
    sql("drop table IF EXISTS mt")
    sql("drop table IF EXISTS dt")

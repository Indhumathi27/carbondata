...
class TestAllOperationsOnMV extends QueryTest with BeforeAndAfterEach {
  ...
  override def beforeEach(): Unit = {
    ...
    sql("create datamap dm1 on table maintable using 'mv' WITH DEFERRED REBUILD as select name,sum(price) " +
  ...
  test("test alter add column on datamaptable") {
    ...
    }.getMessage.contains("Cannot add columns in MV DataMap table default.dm1_table")
  }
  ...
  test("test drop column on maintable") {
    ...
    }.getMessage.contains("Column name cannot be dropped because it exists in mv datamap default.dm1_table")
  }

  test("test alter drop column on datamaptable") {
    intercept[ProcessMetaDataException] {
      sql("alter table dm1_table drop columns(maintable_name)")
    }.getMessage.contains("Cannot drop columns present in MV datamap table default.dm1_table")
  }
  ...
  test("test alter rename table") {
    ...
    }.getMessage.contains("alter rename is not supported for mv datamap table or for tables which have mv datamap")
  ...
  test("test table properties") {
    ...
    sql("drop datamap if exists dm1")
    sql("create datamap dm1 on table maintable using 'mv' WITH DEFERRED REBUILD as select name,price from maintable")
    checkExistence(sql("describe formatted dm1_table"), true, "Local Dictionary Enabled false")
    sql("drop table IF EXISTS maintable")
  ...
  test("test partition table with mv") {
    ...
    sql("create datamap p1 on table par_table using 'mv' WITH DEFERRED REBUILD as select city, id from par_table")
    sql("rebuild datamap p1")
    intercept[MalformedCarbonCommandException] {
      sql("alter table par_table drop partition (city='def')")
    }.getMessage.contains("Drop Partition is not supported for mv datamap table or for tables which have mv datamap")
    sql("drop datamap if exists p1")
  }
  ...
  test("test direct load to mv datamap table") {
    ...
    sql("insert into table maintable select 'abc',21,2000")
    sql("drop datamap if exists dm1")
    sql("create datamap dm1 on table maintable using 'mv' WITH DEFERRED REBUILD as select name " +
        "from maintable")
    sql("rebuild datamap dm1")
    intercept[UnsupportedOperationException] {
      sql("insert into dm1_table select 2")
    }.getMessage.contains("Cannot insert/load data directly into pre-aggregate/mv table")
    sql("drop table IF EXISTS maintable")
  }

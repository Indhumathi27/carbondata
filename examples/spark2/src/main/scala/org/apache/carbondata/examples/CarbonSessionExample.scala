...
//    spark
//      .sql(
//        "create table test(person struct<detail:struct<name:string,age:int>>) stored by " +
//        "'carbondata'")
//    spark.sql("insert into test values('rg$10')")
//    spark.sql("select person.detail.name,person.detail.age from test").show()

//    spark.sql("DROP TABLE IF EXISTS test")
//    spark.sql("CREATE TABLE test (person struct<detail:array<string>,age:struct<no:int,word:string>>) STORED BY 'org.apache.carbondata.format'")
//    spark.sql("insert into test values('car$10')")
//    spark.sql("select person.age.no from test").show()


//    spark.sql("DROP TABLE IF EXISTS table1")
//    spark
//      .sql(
//        "create table table1 (roll int,person Struct<detail:Struct<name:string,age:int>>) stored by " +
//        "'carbondata'")
//    spark
//      .sql(
//        "load data inpath '/home/root1/sofs.csv' into table table1 options('delimiter'=','," +
//        "'quotechar'='\"','fileheader'='roll,person','complex_delimiter_level_1'='$'," +
//        "'complex_delimiter_level_2'='&')")
    spark.sql(" select  person.detail.age from table1 ").show(false)

//    spark.sql("DROP TABLE IF EXISTS table1")
//        spark
//          .sql(
//            "create table table1 (person Struct<detail:string,ph:array<int>>,person1 Struct<detail:string,ph:array<int>>) stored by " +
//            "'carbondata'")
//    spark
//          .sql(
//            "load data inpath '/home/root1/sofa.csv' into table table1 options('delimiter'=','," +
//            "'quotechar'='\"','fileheader'='person,person1','complex_delimiter_level_1'='$'," +
//            "'complex_delimiter_level_2'='&')")
//    spark.sql("select person.detail, person1.ph[0] from table1").show(false)


//    spark.sql("DROP TABLE IF EXISTS test")
//        spark.sql("CREATE TABLE test (person string,age int) STORED BY 'org.apache.carbondata.format'")
//        spark.sql("insert into test values('car',10)")
//        spark.sql("select person,age from test").show()
